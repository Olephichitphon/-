...
async function test() {
  const img = await loadImage('public/assets/logos/insurance/individual/muang-thai.png');
  console.log(`muang-thai dimensions: ${img.width}x${img.height}`);
  ...
  for (let y = 0; y < Math.min(15, img.height); y++) {
    let rowPixels = [];
